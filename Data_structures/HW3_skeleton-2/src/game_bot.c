...
int get_max(TreeNode* node) {
    if (!node || node->num_children <= 0 || get_game_status(node->game_state) != IN_PROGRESS) {
        return node->game_state->evaluation; // Leaf node evaluation
    }

    int max_value = -100000;
    for (int i = 0; i < node->num_children; i++) {
        if (node->children[i]) { // Validate child node
...
int best_move(TreeNode* root) {
    ...
    eval_game_tree(root); // Ensure the tree is evaluated before finding the best move
    ...
    for (int i = 0; i < gs->width; i++) {
        if (moves[i] && root->children[i]) { // Validate available move and child node
...
void eval_game_tree(TreeNode* root) {
    ...
    if (root->num_children <= 0) { // Leaf node
    ...
    for (int i = 0; i < root->num_children; i++) {
        if (root->children[i]) {
            eval_game_tree(root->children[i]); // Recursive evaluation
        }
    }

    int best_evaluation = -100000; // Assuming we are maximizing
    for (int i = 0; i < root->num_children; i++) {
        if (root->children[i]) {
    ...
    root->game_state->evaluation = best_evaluation; // Update root evaluation
}
